...
### `OllamaRequestFull` struct introduction

The `options` key in Ollama requests must be omitted entirely when `num_ctx` is
`None` rather than serialized as `null`. A nested `OllamaOptions` struct with
`#[serde(skip_serializing_if = "Option::is_none")]` on `num_ctx` achieves this
cleanly without conditional JSON construction.

### RFC 1123 label validation in `validate_for_acp`

Agent names in `acp.agents` must comply with RFC 1123 DNS label syntax
(`^[a-z0-9]([a-z0-9\-]{0,61}[a-z0-9])?$`) so they can be used directly as
service identifiers in multi-agent routing. The validator uses the `regex` crate
which is already a project dependency.

### `tool_invocations` table uses ULID primary keys

ULID values are used for the `id` column (consistent with the rest of the
storage layer) to preserve insertion-order sortability without requiring a
separate `created_at` index. RFC-3339 timestamps in the `timestamp` column
provide human-readable audit trails.

### `chat_history_file = None` falls back to platform data directory

Rather than hard-coding `~/.xzatoma/chat_history`, the fallback uses the
`directories` crate (`ProjectDirs::from("com", "xbcsmith", "xzatoma")`) so the
path follows OS conventions on macOS (`~/Library/Application Support/...`),
Linux (`~/.local/share/...`), and Windows (`%APPDATA%\...`).
