...
This plan adds Agent Communication Protocol (ACP) support to XZatoma in a phased,
incremental way that fits the current Rust CLI architecture and reuses existing
conversation, storage, provider, watcher, and MCP-related foundations where
they are a good fit. The goal is to let XZatoma act as an ACP server first,
exposing one or more XZatoma agents through a versioned REST API with sync,
async, and streaming run modes, then expand into stronger session handling,
await/resume behavior, discovery metadata, and production hardening.

The recommended implementation order is:

1. ACP protocol domain model and internal run abstraction
...
This phased order minimizes architecture risk by establishing ACP data types and
state transitions before introducing HTTP transport and long-running execution
coordination. It also keeps the initial delivery aligned with the ACP REST API
surface while avoiding premature abstraction or a large rewrite of existing
...
7. There is no explicit await/resume mechanism in the ACP sense, even though the
   project has adjacent concepts such as interactive flows and persisted history.
...
Define strongly typed ACP protocol models based on the ACP specification for:

- agent names and manifests
- messages and message parts
- run create/resume requests
...
Create adapter helpers that convert existing provider/agent conversation message
shapes into ACP `Message` output so later phases can expose current XZatoma
behavior through ACP without rewriting the provider layer.
...
history does not diverge from the current XZatoma storage model. Define a
...
**Mitigation:** Introduce a minimal coordinator-owned await boundary and document
any limitations in the first release instead of forcing deep execution rewrites
early.
