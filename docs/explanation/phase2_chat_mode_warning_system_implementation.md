...
- `/context summary --model gpt-4` → `ContextSummary { model: Some("gpt-4") }`
- `/context summary -m claude-3` → `ContextSummary { model: Some("claude-3") }`
...
- `test_parse_context_summary_with_complex_model_name()` - Complex model names (e.g., gpt-4-turbo-preview)
...
```
> /context summary
Summarizing conversation using model: gpt-4...
...
```
> /context summary --model gpt-3.5-turbo
Summarizing conversation using model: gpt-3.5-turbo...
