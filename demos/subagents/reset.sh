#!/bin/sh
# Subagents Demo - Reset
#
# Removes all generated state under tmp/ and returns the demo to its initial
# state. Plan files and configuration are never modified by this script.
#
# Usage:
#   sh ./reset.sh
...
# Remove the subagent conversation persistence database
if [ -f tmp/subagent_conversations.db ]; then
    rm -f tmp/subagent_conversations.db
    echo "  Removed tmp/subagent_conversations.db"
fi
