...
# state. Fixture files, input data, and configuration are never modified.
#
# Usage:
#   sh ./reset.sh
...
# Remove the watcher structured log
if [ -f tmp/watcher.log ]; then
    rm -f tmp/watcher.log
    echo "  Removed tmp/watcher.log"
fi
