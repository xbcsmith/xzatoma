...
# read_results.sh
#
# Reads result events from the XZatoma XZepr watcher output topic and
# pretty-prints each result as it arrives.  Blocks until Ctrl+C.
#
# Usage:
...
Start the Redpanda stack first:
  docker compose -f docker-compose.redpanda.yaml up -d"
fi
...
#
# With jq:   parse .value (the result JSON) and pretty-print it.
