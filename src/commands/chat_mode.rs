...
impl ChatMode {
    ...
    ///
    /// ```
    /// use xzatoma::commands::chat_mode::ChatMode;
    ///
    /// let mode = ChatMode::from_str("planning").unwrap();
...
impl SafetyMode {
    ...
    /// # Arguments
    ///
    /// * `s` - String representation of the mode ("confirm", "always", "yolo", or "never")
    ///
    /// # Returns
    ///
    ...
    ///
    /// ```
    /// use xzatoma::commands::chat_mode::SafetyMode;
    ///
    /// let mode = SafetyMode::from_str("confirm").unwrap();
    /// assert_eq!(mode, SafetyMode::AlwaysConfirm);
    /// ```
    pub fn from_str(s: &str) -> Result<Self, String> {
        match s.to_lowercase().as_str() {
...
impl ChatModeState {
    ...
    ///
    /// ```
    /// use xzatoma::commands::chat_mode::{ChatMode, SafetyMode, ChatModeState};
    ///
...
mod tests {
    ...
    fn test_chat_mode_from_str_write() {
        let mode = ChatMode::from_str("write").unwrap();
        assert_eq!(mode, ChatMode::Write);
    }

    #[test]
    fn test_chat_mode_from_str_case_insensitive() {
        assert_eq!(ChatMode::from_str("PLANNING").unwrap(), ChatMode::Planning);
        assert_eq!(ChatMode::from_str("Write").unwrap(), ChatMode::Write);
    }

    #[test]
    fn test_chat_mode_from_str_invalid() {
        assert!(ChatMode::from_str("invalid").is_err());
    }
    ...
    fn test_safety_mode_from_str_confirm_variants() {
        assert_eq!(
            SafetyMode::from_str("confirm").unwrap(),
            SafetyMode::AlwaysConfirm
        );
        assert_eq!(
            SafetyMode::from_str("always").unwrap(),
            SafetyMode::AlwaysConfirm
        );
        assert_eq!(
            SafetyMode::from_str("safe").unwrap(),
            SafetyMode::AlwaysConfirm
        );
        assert_eq!(
            SafetyMode::from_str("on").unwrap(),
            SafetyMode::AlwaysConfirm
        );
    }
    ...
    fn test_safety_mode_from_str_yolo_variants() {
        assert_eq!(
            SafetyMode::from_str("yolo").unwrap(),
            SafetyMode::NeverConfirm
        );
        assert_eq!(
            SafetyMode::from_str("never").unwrap(),
            SafetyMode::NeverConfirm
        );
        assert_eq!(
            SafetyMode::from_str("off").unwrap(),
            SafetyMode::NeverConfirm
        );
    }
    ...
    fn test_safety_mode_from_str_invalid() {
        assert!(SafetyMode::from_str("invalid").is_err());
    }
